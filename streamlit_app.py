...
#                 try:
#                     resp = requests.post(f"https://30631d9db687.ngrok-free.app/init-location?pincode={pincode_input}")
#                     if resp.status_code == 200:
#                         st.session_state.pincode = pincode_input
#                         st.success(f"✅ Location set: {pincode_input}")
#                         st.rerun()
#                     else:
#                         st.error("Failed to set location. Check server.")
...
#     user_query = st.text_input("Describe your need:", placeholder="e.g., I want chocolates and milk")
...
#                 try:
#                     # Extract keywords
#                     kw_resp = requests.get(f"https://30631d9db687.ngrok-free.app/keywords?query={requests.utils.quote(user_query)}")
...
#                                 f"https://30631d9db687.ngrok-free.app/search?keyword={kw}&pincode={st.session_state.pincode}"
#                             )
...
#                     # Show results
#                     if all_results:
#                         st.markdown(f"### 🎉 Found {len(all_results)} products")
#                         def extract_price(p):
#                             return float(''.join(filter(str.isdigit, p.replace('₹', '')))) if any(c.isdigit() for c in p) else float('inf')
...
#                                         st.write("🖼️")
#                                 with col2:
#                                     st.markdown(f"**[{r['source']}]** [{r['name']}]({r['url']})")
#                                     st.markdown(f"💰 {r['price']} | ~~{r['mrp']}~~ | {r['quantity']} | 🚚 {r['delivery_time']}")
#                                 st.markdown("---")
#                     else:
